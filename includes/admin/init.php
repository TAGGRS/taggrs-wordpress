...
function tggr_admin_notices()
{
    if ($error = get_transient('tggr_settings_error')) {
        ...
        delete_transient('tggr_settings_error');
    }
}
...
function tggr_options_sanitize($input)
{
    ...
    // Sanitize container ID voor enhanced tracking
    $input['enhanced_tracking_container_id'] = isset($input['enhanced_tracking_container_id'])
        ? sanitize_text_field($input['enhanced_tracking_container_id'])
        : '';

    // Verwijder oude v2-sleutels als ze nog in de database staan
    unset($input['enhanced_tracking_v2']);
    unset($input['enhanced_tracking_v2_container_id']);
...
function tggr_enhanced_tracking_cb($args)
{
    $options = get_option('tggr_options');
    $disabled = !isset($options['tggr_url_toggle']) || $options['tggr_url_toggle'] == '';
    $container_id = isset($options['enhanced_tracking_container_id']) ? $options['enhanced_tracking_container_id'] : '';

    echo '<div id="enhanced_tracking_section" style="' . ($disabled ? 'opacity: 0.7;' : '') . '">';
    echo '<p class="description" style="margin-top: 10px;"><b>TAGGRS Container Identifier</b></p>';
    echo '<input type="text" id="enhanced_tracking_container_id" name="tggr_options[enhanced_tracking_container_id]" '
        . ($disabled ? 'disabled' : '')
        . ' style="width:350px;" value="' . esc_attr($container_id) . '" />';
    echo '</div>';
    echo '<p class="description"><i>Enhanced Tracking can only be used when you have entered a subdomain above. Fill in your TAGGRS Container Identifier to activate it.</i></p>';
}
...
function tggr_settings_init()
{
    ...
    add_settings_field(
        'tggr_enhanced_tracking',
        'Enhanced Tracking',
        'tggr_enhanced_tracking_cb',
        'wc-gtm-settings',
        'tggr_section_gtm'
    );
