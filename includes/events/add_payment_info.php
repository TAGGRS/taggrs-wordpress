<?php
if ( ! defined( 'ABSPATH' ) ) exit;

// Classic checkout: add_payment_info event
// Note: For Blocks checkout, this is handled by js/wc-blocks-checkout.js
function tggr_add_payment_info()
{
    $options = get_option('tggr_options');

    if (!isset($options['add_payment_info']) || !$options['add_payment_info']) {
        return;
    }

    $current_user = wp_get_current_user();
    $hashed_email = '';
    $email = '';
    
    if ($current_user->exists()) {
        $hashed_email = hash('sha256', $current_user->user_email);
        $email = $current_user->user_email;
    }

    $total_value = 0;
    $items = array();

    // Controleer of we in de checkout zijn
    if (is_checkout()) {
        $cart = WC()->cart;
        if ($cart) {
            $items = tggr_format_cart_items($cart);
            $total_value = $cart->cart_contents_total;
        }
    }
?>
    <script>
        // Only track for classic checkout (Blocks checkout is handled separately)
        if (!document.querySelector('.wc-block-checkout')) {
            window.dataLayer = window.dataLayer || [];
            
            // Track when payment method is selected
            jQuery(document).ready(function($) {
                var lastTrackedPaymentMethod = null;
                
                function trackPaymentInfo() {
                    var paymentMethod = $('input[name="payment_method"]:checked').val() || '';
                    
                    // Don't track if no method selected or same as last tracked
                    if (!paymentMethod || paymentMethod === lastTrackedPaymentMethod) {
                        return;
                    }
                    
                    dataLayer.push({
                        'event': 'add_payment_info',
                        'ecommerce': {
                            'currency': '<?php echo esc_js(get_woocommerce_currency()); ?>',
                            'value': <?php echo esc_js($total_value); ?>,
                            'items': <?php echo wp_json_encode($items); ?>,
                            'payment_type': paymentMethod
                        },
                        'user_data': {
                            'email': '<?php echo esc_js($email); ?>',
                            'email_hashed': '<?php echo esc_js($hashed_email); ?>'
                        }
                    });
                    
                    lastTrackedPaymentMethod = paymentMethod;
                }
                
                // Track when payment method is selected
                $(document.body).on('change', 'input[name="payment_method"]', function() {
                    trackPaymentInfo();
                });
                
                // Also track if payment method is already selected
                if ($('input[name="payment_method"]:checked').length > 0) {
                    setTimeout(trackPaymentInfo, 500);
                }
            });
        }
    </script>

<?php
}

add_action('woocommerce_review_order_before_payment', 'tggr_add_payment_info');
?>